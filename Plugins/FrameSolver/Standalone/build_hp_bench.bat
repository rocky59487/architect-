@echo off
REM Builds hp_bench.exe: HP-FEM seeded-session vs LDLT per-frame timing (diagnostic, not a gate leg).
REM Same FrameCore translation units as build.bat (incl. HpSession.cpp), but main.cpp -> hp_bench.cpp.
...
if not exist "%EIGEN%\Eigen" ( echo [build_hp_bench] Eigen include root not found: "%EIGEN%" & exit /b 1 )
...
if "!VSDIR!"=="" ( echo [build_hp_bench] could not locate Visual Studio via vswhere. & exit /b 1 )
for %%d in ("%VSWHERE%") do set "PATH=%%~dpd;%PATH%"
call "!VSDIR!\VC\Auxiliary\Build\vcvars64.bat" >nul
if errorlevel 1 ( echo [build_hp_bench] vcvars64 failed & exit /b 1 )

pushd "%ROOT%"
if not exist "Standalone\obj_hp_bench" mkdir "Standalone\obj_hp_bench"
...
cl /nologo /EHsc /std:c++17 /O2 /MD /utf-8 /DEIGEN_MPL2_ONLY /DFRAMECORE_BUILD_SHA=\"!GITSHA!\" ^
   /Fe:Standalone\hp_bench.exe ^
   /Fo:Standalone\obj_hp_bench\ ^
   /I"%EIGEN%" ^
   ...
   Standalone\hp_bench.cpp
if errorlevel 1 ( echo [build_hp_bench] COMPILE FAILED & popd & exit /b 1 )

echo [build_hp_bench] OK -^> Standalone\hp_bench.exe
echo.
"Standalone\hp_bench.exe"
set "RC=%errorlevel%"
popd
exit /b %RC%
